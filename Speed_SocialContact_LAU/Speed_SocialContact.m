...
%    run check tag, divide group, filling the missing data, then check the social contact.
...
%       result File to save the 'ContactTable','Real_Contact_List_alley','Real_Contact_List_cubic','Real_Contact_List_feed','Real_Contact_List_wholebarn'
...
data=readFAfile('E:\Work\Data\FA_20200915T000000UTC');

%IDlist=RightGroup;
starttime='15-Sep-2020 00:00:00';   %GMT TIME. Sweden time need to +2 hours
endtime='15-Sep-2020 23:59:59';
...
load('matrix_area2.mat')            %Area defination


GroupFileName='Groups_20200915.mat';
filledIndividualIinterval_FileName='Filled_individual_interval_20200915.mat';
resultFileName='ContactResult_20200915_LeftGroup.mat';
...
save(GroupFileName,'LeftGroup','RightGroup');

%load('Groups_20200915.mat');

IDlist=LeftGroup;%%%%%%%Change the interested group for Swedish farm. For Dutch farm, use IDlist=MovingID;
...
for idIdx=1:length(IDlist)
    ...
    for i=1:18      % call Area defination, seprate areas as keycow position
    ...
    Real_Contact_List_cubic{1,idIdx}=KeycowContactOrdered{1,idIdx}(find(KeycowContactOrdered{1,idIdx}.Area>=3 & KeycowContactOrdered{1,idIdx}.Area<=12),:);
    Real_Contact_List_feed{1,idIdx}=KeycowContactOrdered{1,idIdx}(find(KeycowContactOrdered{1,idIdx}.Area>=1 & KeycowContactOrdered{1,idIdx}.Area<=2),:);
    Real_Contact_List_alley{1,idIdx}=KeycowContactOrdered{1,idIdx}(find(KeycowContactOrdered{1,idIdx}.Area>=13 & KeycowContactOrdered{1,idIdx}.Area<=18),:);
    ...
    Contact_List_alley=Real_Contact_List_alley;
...
for idIdx=1:length(IDlist)
    ...
    uniquenum_alley=unique(Real_Contact_List_alley{1,idIdx}.pairIdx);
        ...
        for idxPairs=1:length(uniquenum_alley)
            if sum(Real_Contact_List_alley{1,idIdx}.pairIdx==uniquenum_alley(idxPairs))<=realContactThreshold
            Real_Contact_List_alley{1,idIdx}(ismember(Real_Contact_List_alley{1,idIdx}.pairIdx,uniquenum_alley(idxPairs)),:)=[];
            end
        end
...
% For each individual, each day, duration time  and how many cows she had interaction when she in alley.
Real_Contact_List_alley_duration(idxID)=height(Real_Contact_List_alley{1,idxID});
Real_Contact_List_alley_NumFriends(idxID)=length(unique(Real_Contact_List_alley{1,idxID}.Pair_Second(:,1)));

ID1List(idxID,1)=Filled_individual_interval{idxID}.id1(1);      %% Change ID to ID1
end

varNamesContactTable = {'id1','Whole Duration','Whole numFriends','Cubic Duration','Cubic numFriends','Feed Duration','Feed numFriends','Alley Duration','Alley numFriends'};
ContactTable=table(ID1List,Real_Contact_List_wholebarn_duration',Real_Contact_List_wholebarn_NumFriends',Real_Contact_List_cubic_duration',Real_Contact_List_cubic_NumFriends',Real_Contact_List_feed_duration',Real_Contact_List_feed_NumFriends',Real_Contact_List_alley_duration',Real_Contact_List_alley_NumFriends','VariableNames',varNamesContactTable);

save(resultFileName,'ContactTable','Real_Contact_List_alley','Real_Contact_List_cubic','Real_Contact_List_feed','Real_Contact_List_wholebarn','-v7.3','-nocompression');
